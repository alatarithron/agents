...
Canonical source: `AGENTS.md` in github.com/alatarithron/agents. Resolve global instruction symlinks or the local pointer generated by `wire.sh` to locate this file. Paths to `reference/` below are relative to its real containing directory, not the current project or the symlink directory.
