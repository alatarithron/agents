...
Canonical source: `AGENTS.md` in github.com/alatarithron/agents. Resolve the global instruction symlinks created by `wire.sh`, or the absolute path it appends to Hermes `SOUL.md`, to locate this file. Paths to `reference/` below are relative to its real containing directory, not the current project or the symlink directory.
...
- The full policy — what to record, what never to record, and how to keep it current — is in `reference/PROJECT-MEMORY-POLICY.md`. Read it before creating or restructuring a project's memory.
...
For a security- or privacy-focused review, read `reference/SECURITY-AND-PRIVACY.md` first and use its verification checklist. For language-specific conventions and detailed engineering practices, read `reference/DEVELOPMENT-GUIDELINES.md`. These reference files are not loaded automatically — open them when the task warrants the depth.
